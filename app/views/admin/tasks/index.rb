class Views::Admin::Tasks::Index < Views::Base
  ...
  def page_content
    ...
    render Views::Admin::Tasks::List.new(user: @user, tasks: @tasks, show_done: @show_done)

    div(class: "mt-8 text-center") do
      if @show_done
        link_to "← Back to pending tasks", admin_user_tasks_path(@user), class: "text-sm text-gray-400 hover:text-gray-600 hover:underline"
      else
        link_to "Show completed tasks", admin_user_tasks_path(@user, done: true), class: "text-sm text-gray-400 hover:text-gray-600 hover:underline"
      end
    end
  end
end
