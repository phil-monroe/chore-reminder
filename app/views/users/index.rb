class Views::Users::Index < Views::Base
  ...
  def page_content
    div(class: "flex items-center justify-between mb-6") do
      ...
      link_to "New user", new_user_path, class: "bg-blue-600 text-white px-3 py-1.5 rounded-md text-sm hover:bg-blue-700"
    end
      ...
      div(class: "space-y-2") do
        @users.each do |user|
          div(class: "bg-white border border-gray-200 rounded-lg p-4 flex items-center justify-between") do
            div do
              link_to user.name, user_path(user), class: "font-medium text-gray-900 hover:underline"
