class Views::Tasks::List < Components::Base
  ...
  def view_template
    div(id: "tasks", class: "space-y-2") do
      if @tasks.empty?
        p(class: "text-gray-500 text-sm") { "No tasks yet — add one below." }
      else
  ...
  def task_row(task)
    div(class: "bg-white border border-gray-200 rounded-lg p-3 flex items-center justify-between gap-3 #{"opacity-50" if task.done}") do
      ...
      div(class: "flex items-center gap-1") do
        button_to "done", toggle_done_user_task_path(@user, task), method: :patch,
          class: "text-xs px-2 py-1 rounded #{task.done ? "bg-gray-200 text-gray-700" : "bg-green-600 text-white hover:bg-green-700"}"
        button_to "↑", move_higher_user_task_path(@user, task), method: :patch,
