class Admin::UsersController < Admin::BaseController
  ...
  def new
    @user = User.new
    render Views::Admin::Users::Form.new(user: @user)
  end
  ...
  def create
    ...
    if @user.save
      ...
      render Views::Admin::Users::Form.new(user: @user), status: :unprocessable_content
    end
  end
  ...
  def conversation
    ...
    render Views::Admin::Users::Conversation.new(user: @user, messages: @messages)
  end
  ...
  def send_inbound_message
    run_sms_command(success_notice: nil, failure_prefix: "Failed to send reply",
      redirect_path: conversation_admin_user_path(@user)) do
  ...
  def run_sms_command(success_notice:, failure_prefix:, redirect_path: admin_user_path(@user))
    yield
