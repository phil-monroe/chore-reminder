...
class Mcp::Tools::CreateTaskDefinition < MCP::Tool
  ...
  def self.call(name:, time_of_day:, server_context:, user_id: nil, description: nil, recurrence_days: nil)
    user = resolve_user(user_id, server_context: server_context)
    task_definition = user.task_definitions.new(name: name, description: description, time_of_day: time_of_day, recurrence_days: recurrence_days || [])
