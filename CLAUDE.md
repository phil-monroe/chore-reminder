...
The one exception is the mounted GoodJob dashboard at `/admin/good_job` (`config/routes.rb`): its engine controllers don't inherit from `Admin::BaseController`, or `ApplicationController` at all, so a `before_action` can't gate them. Instead, the `mount GoodJob::Engine` line itself carries `constraints: AdminSessionConstraint.new` (`app/services/admin_session_constraint.rb`) — a routing constraint checking the same `session[Admin::BaseController::SESSION_KEY]` at the routing layer, before any controller runs. A failed constraint just means the route doesn't match, which alone would 404 rather than redirect to `/login` like every other unauthenticated `/admin/...` request gets — `config/routes.rb` adds a `match "/admin/good_job(/*path)", to: redirect("/login")` immediately after the `namespace :admin` block to cover exactly that case. `SessionsController`'s own routes (`/login`, `/logout`) are deliberately outside `/admin` entirely, so reaching the login form itself never requires already being logged in.
