...
Every `Admin::` controller (e.g. `Admin::UsersController`) inherits from `Admin::BaseController`, whose `before_action :require_authenticated!` redirects to `/login` unless `session[:admin_authenticated]` is set — the actual gate, replacing what used to be a Rack middleware. `Admin::BaseController::SESSION_KEY` is the single source of truth for that session key's name, referenced both by `SessionsController#create` (setting it) and by `AdminSessionConstraint` (checking it, see below). All admin routes live under `namespace :admin do ... end` in `config/routes.rb`, with views under `Views::Admin::` (e.g. `Views::Admin::Users::Show`) to match.

The whole `namespace :admin` block is also wrapped in `constraints(AdminSessionConstraint.new) do ... end` (`app/services/admin_session_constraint.rb`) — a routing constraint checking the same `session[Admin::BaseController::SESSION_KEY]` at the routing layer, before any controller runs. This is defense-in-depth for the `Admin::` controllers (which already gate themselves via `before_action`) but is the *only* gate for the mounted GoodJob dashboard at `/admin/good_job`, since its engine controllers don't inherit from `Admin::BaseController`, or `ApplicationController` at all, so a `before_action` can't reach them. A failed constraint just means the route doesn't match, which alone would 404 rather than redirect to `/login` like every other unauthenticated `/admin/...` request gets — `config/routes.rb` adds a single catch-all `match "/admin(/*path)", to: redirect("/login")` immediately after the `namespace :admin` block to cover every path under it. `SessionsController`'s own routes (`/login`, `/logout`) are deliberately outside `/admin` entirely, so reaching the login form itself never requires already being logged in.
